...
class SystemCheck
{
    ...
    public function checkAndChangeCookiesTable()
    {
        ...
        $cookieTableColumnCookieExpiryResult = $wpdb->get_results('
            SELECT
            ...
            WHERE
                ...
                AND
                `TABLE_NAME` = "'.esc_sql($tableNameCookies).'"
                AND
                `COLUMN_NAME` = "cookie_expiry"
        ');

        if (!empty($cookieTableColumnCookieExpiryResult[0])) {

            if (!empty($cookieTableColumnCookieExpiryResult[0]->CHARACTER_MAXIMUM_LENGTH) && $cookieTableColumnCookieExpiryResult[0]->CHARACTER_MAXIMUM_LENGTH !== '255') {
